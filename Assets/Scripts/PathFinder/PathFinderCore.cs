...
//API
		...
		public Node GetNearSafePoint(Transform enemy, Transform aggressor, float wEnemy = 1f, float wAggressor = -1f ){
	    	Vector2 dirEnemy, dirAggressor;
			float MinMax = Mathf.Infinity, curDist;
			Node result = listPoint[0];
			Node node = null;
			for(int i = 0; i < 10; i++){
				node = listPoint[Random.Range(0, listPoint.Count)];
				...
				if(!CheckVisibledNode(currentNode)){
					for(int i=0; i < relation.Count; i++){
						if(relation[i].LeftNode.Point == currentNode.Point){
							...
							if(relation[i].LeftNode == null) {Debug.Log("null");}
							if(finishNode == null) {Debug.Log("null");}
